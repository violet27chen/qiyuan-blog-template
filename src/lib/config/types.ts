...
export interface RouterItem {
  ...
  icon?: string;
  children?: RouterItem[];
  /** 是否显示在导航栏，默认 true；设为 false 可隐藏该菜单项（用于可选功能默认关闭） */
  enabled?: boolean;
}
...
export type CommentProvider = 'remark42' | 'giscus' | 'waline' | 'twikoo' | 'custom' | 'none';
...
/**
 * KV-based pageview counter (EdgeOne Pages KV).
 * Self-hosted, privacy-friendly counter that stores per-page and site-wide
 * view counts in an EdgeOne KV namespace via the `/api/views` edge function.
 */
export interface PageviewsConfig {
  /** Master switch for the KV pageview counter */
  enabled: boolean;
  /** Show per-article view count on article pages / cards */
  article_page_views?: boolean;
  /** Show site-wide total views in the footer Site Stats */
  footer_site_stats?: boolean;
}
...
export interface SakuraFallConfig {
  /** 桌面端花瓣数量 */
  intensity: number;
  /** 移动端花瓣数量 */
  mobileIntensity: number;
...
export interface AnimeEffectsConfig {
  enabled: boolean;
  features: AnimeFeatures;
  sakura: SakuraFallConfig;
}
...
// =============================================================================
// Resources Hub (resource sharing)
// =============================================================================
...
export interface ResourcesPageConfig {
  ...
  label?: string;
  icon?: string;
  /** Optional owner user id filter for public resource listing */
...
export interface SiteYamlConfig {
  ...
  /** Cloud-drive resources hub — synced from the resource site API (default resources.qiyuan.icu, override via RESOURCES_API_BASE) */
  resources?: ResourcesPageConfig;
  christmas?: ChristmasConfig;
  /** 二次元动漫特效（樱花飘落、点击爆裂、鼠标拖尾） */
  anime?: AnimeEffectsConfig;
...
export interface TempMailConfig {
  /** 总开关 */
  enabled?: boolean;
  /** 白嫖免费域名（NS 托管到 Cloudflare，Email Routing catch-all 指向收信 Worker） */
  domain?: string;
  /** 可选题域名列表（用户自行选择其一） */
  domains?: string[];
  /** 邮件/地址保留时长（小时），默认 24 */
