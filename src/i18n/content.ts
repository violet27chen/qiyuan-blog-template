...
/**
 * Look up a translated site-level field (subtitle, description) from the YAML content config.
 * Returns undefined if no translation is defined.
 */
export function getContentSiteField(locale: string, field: string): string | undefined {
  const site = config[locale]?.site;
  if (!site) return undefined;
  return site[field as keyof typeof site];
}
