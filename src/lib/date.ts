...
export function parseDateInSiteTimezone(dateString: string): Date {
  ...
  let result = toDate(dateString, { timeZone: siteTimezone });

  // If it fails (e.g., english formats like "May 25 2026 10:00"), fall back to JS Date parsing
  if (Number.isNaN(result.getTime())) {
    ...
    if (!Number.isNaN(fallbackDate.getTime())) {
      ...
      const hr = String(fallbackDate.getHours()).padStart(2, '0');
      const min = String(fallbackDate.getMinutes()).padStart(2, '0');
      const sec = String(fallbackDate.getSeconds()).padStart(2, '0');
      const dateStr = `${y}-${m}-${d} ${hr}:${min}:${sec}`;
      result = fromZonedTime(dateStr, siteTimezone);
    }
  }
