...
import { resolveLocalizedString } from '@lib/i18n/announcement';
...
function TimelineItem({
  ...
  const content = resolveLocalizedString(announcement.content, locale);
  const linkText = announcement.link?.text ? resolveLocalizedString(announcement.link.text, locale) : undefined;
    ...
    <div className="relative flex gap-2 md:gap-4">
      ...
      >
        ...
        <div className="p-3 pl-4 md:p-4 md:pl-5">
          ...
          {/* Content */}
          <p className="mt-1.5 text-muted-foreground text-xs leading-relaxed md:mt-2 md:text-sm">{content}</p>
          ...
          {announcement.link && (
            ...
            >
              {linkText ?? t('announcement.learnMore')}
